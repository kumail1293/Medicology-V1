...
export const emailTemplatesTable = pgTable(
  "email_templates",
  {
    ...
    bodyBlocks: jsonb("body_blocks").$type<any[]>().notNull().default([]), // block[] from the visual builder
    status: text("status").notNull().default("draft"), // draft | published | archived
    version: integer("version").notNull().default(1),
    versions: jsonb("versions").$type<any[]>().notNull().default([]), // [{version, subject, bodyBlocks, changedBy, changedAt}]
    variables: jsonb("variables").$type<string[]>().notNull().default([]), // allowed {{vars}} for validation
